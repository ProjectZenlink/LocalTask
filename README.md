...
function JoinInner() {
  ...
  // v65 ①:进页预登录 —— 首个 Turnstile token 一到手就后台换匿名会话,
  // 并重置组件取新 token 备用;随后空载预热 lead-intake(杀 Edge 冷启动)。
  // 提交时会话已在、token 已新,只剩一跳函数调用。
  useEffect(() => {
    if (phase !== 'form' || !captchaToken || preAuthRef.current) return
    preAuthRef.current = true
    void (async () => {
      ...
      if (!sess.session) {
        ...
        if (aerr) { preAuthRef.current = false; return }
        captchaRef.current?.reset()
      }
      void supabase.functions.invoke('lead-intake', { body: { ping: true } }).catch(() => {})
    })()
  }, [phase, captchaToken])
  ...
  async function submit() {
    ...
    try {
      ...
      const { data: sess } = await supabase.auth.getSession()
      if (!sess.session) {
        // 兜底路径(预登录未及生效):老三步 —— 换会话 → 重置 → 等新 token
        const { error: aerr } = await supabase.auth.signInAnonymously({
      ...
      // Meta Pixel:Lead 转化事件(v66;像素被拦/未加载时 fbq 不存在,安全空转)
      ;(window as unknown as { fbq?: (...a: unknown[]) => void }).fbq?.('track', 'Lead')
      // v65 ①:直用函数返回的 conversation_id 秒进聊天;lead 详情后台补水
      const rr = data as { conversation_id?: string | null } | null
      if (rr?.conversation_id) setFastConv(rr.conversation_id)
