...
const COPY = {
  en: {
    ...
    waLabel: 'WhatsApp (with country code) — required',
    waHint: 'Enter a WhatsApp your AM can actually reach — task assignments and check-in confirmations depend on it.', dob: 'Date of birth',
  ...
  zh: {
    ...
    fullName: '法定全名',
    fullNameHint: '必须与证件上的姓名完全一致；请勿填写中间名。',
    waLabel: 'WhatsApp（含国家区号）——必填',
    waHint: '请填写能联系到你的 WhatsApp——AM 找你派任务和确认签到都靠它。', dob: '出生日期',
...
export default function BuildProfile() {
  ...
  async function save() {
    setError(null)
    if (!whatsapp.trim() || !fullName || !dob || !address || !city || !state || !zip) {
