...
import type { AmNote, AccountRecord } from '../types/database'
...
const COPY = {
  zh: {
    expiring: '手机号即将到期', none: '暂无提醒,也没有未完成的备忘。',
    memos: '备忘录', add: '添加', placeholder: '例:明天联系 James 做 Shopify',
    daysLeft: (d: number) => d <= 0 ? '今天到期' : `${d} 天后到期`,
  },
  en: {
    expiring: 'Phone numbers expiring', none: 'No reminders and no open memos.',
...
export default function Bell({ amId }: { amId: string }) {
  ...
  const badge = expiring.length + notes.filter(x => x.remind_on && x.remind_on <= today).length
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-10 z-40 w-80 rounded-xl border border-hair bg-surface p-4 shadow-[0_16px_48px_rgba(26,32,30,0.18)]">
          ...
          {notes.length === 0 && expiring.length === 0 && (
