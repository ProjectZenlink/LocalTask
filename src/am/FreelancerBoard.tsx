...
const COPY = {
  zh: {
    ...
    accept: '验收', reaccept: '重新验收', markDone: '标记完成', stDone: '完成', stNot: '未完成', acceptedOn: '验收于', rejectedNote: '被驳回:', records: '账号资料', addRec: '＋ 添加账号',
  ...
  en: {
    ...
    accept: 'Accept', reaccept: 'Re-accept', markDone: 'Mark done', stDone: 'Done', stNot: 'Not done', acceptedOn: 'Accepted', rejectedNote: 'Rejected:', records: 'Account records', addRec: '＋ Add account',
...
export default function AmFreelancerBoard() {
  ...
  const [reopenFor, setReopenFor] = useState<AcceptanceRow | null>(null)
  const [resolveFor, setResolveFor] = useState<AcceptanceRow | null>(null)
  ...
  async function doReopen(reason: string) {
    const a = reopenFor; setReopenFor(null)
    if (!a) return
    setError(null)
  ...
  async function doResolve() {
    const a = resolveFor; setResolveFor(null)
    if (!a) return
    setError(null)
      ...
      <div className="grid grid-cols-1 gap-x-4 sm:grid-cols-2">
        ...
        <div className="mb-4">
          ...
          <select value={recForm.status} onChange={e => setRecForm({ ...recForm, status: e.target.value })}
            ...
            <option value="pending">{t.rsPending}</option>
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <Card className="p-5">
        {PLATFORMS.map(type => {
          ...
          const state: 'grey' | 'yellow' | 'review' | 'green' | 'reopened' =
            acc?.status === 'reopened' ? 'reopened'
          ...
          const badgeTone = (state === 'green' || state === 'review') ? 'verified' : 'unverified'
          const badgeLabel = state === 'reopened' ? t.stReopen
            : (state === 'green' || state === 'review') ? t.stDone : t.stNot
          return (
            <div key={type} className="border-b border-hair py-3 last:border-b-0">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  {(state === 'yellow' || state === 'grey') && (
                    <Button className="px-3 py-1.5 text-xs" disabled={busyType === type} onClick={() => void accept(type)}>
                      {rej ? t.reaccept : t.markDone}{rate > 0 ? ` +${usd(rate)}` : ''}
                    </Button>
                  )}
                  {state === 'reopened' && acc && (
                    <Button className="px-3 py-1.5 text-xs" onClick={() => setResolveFor(acc)}>{t.resolveBtn}</Button>
                  )}
                  {(state === 'green' || state === 'review') && acc && (
                    <Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => setReopenFor(acc)}>{t.reopenBtn}</Button>
                  )}
              ...
              {state === 'reopened' && acc?.reopened_reason && (
                <p className="mt-1.5 text-xs text-danger-text">{t.reopenNote} {acc.reopened_reason}</p>
              )}
                    ...
                    <div key={r.id} className="mt-2 rounded-lg border border-hair bg-white p-3">
                      <div className="grid grid-cols-1 gap-x-6 gap-y-1.5 text-xs sm:grid-cols-2">
                        ...
                        <p className="flex flex-wrap items-center gap-2"><span className="text-faint">{t.twofa}: </span>{r.twofa ? <><SecretText value={r.twofa} /><TotpCode secret={r.twofa} /></> : '—'}
                          <span className={`font-mono text-[10px] uppercase tracking-wider ${ (r.status ?? 'active') === 'active' ? 'text-verified-text' : ['pending', 'review'].includes(r.status ?? 'active') ? 'text-pending-text' : 'text-danger-text'}`}>
      ...
      <Card className="mt-5 p-5">
        ...
        {otherTasks.length === 0 ? (
          ...
          const state: 'yellow' | 'review' | 'green' | 'reopened' =
            live?.status === 'reopened' ? 'reopened'
            ...
            <div key={ot.id} className="border-b border-hair py-3 last:border-b-0">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  {state !== 'reopened' && <Circles stage={stage} />}
                  <StatusBadge status={state === 'green' ? 'verified' : state === 'reopened' ? 'unverified' : 'pending'}
                    label={state === 'reopened' ? t.stReopen : state === 'green' ? t.stGreen : state === 'review' ? t.stReview : t.stYellow} />
                  <span className="font-mono text-xs text-faint">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {state === 'reopened' && live && (
                    <Button className="px-3 py-1.5 text-xs" onClick={() => setResolveFor(live)}>{t.resolveBtn}</Button>
                  )}
                  {(state === 'green' || state === 'review') && live && (
                    <Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => setReopenFor(live)}>{t.reopenBtn}</Button>
                  )}
                </div>
              </div>
              {rej && state === 'yellow' && (
              ...
              {state === 'reopened' && live?.reopened_reason && (
                <p className="mt-1.5 text-xs text-danger-text">{t.reopenNote} {live.reopened_reason}</p>
              )}
            </div>
          )
        })}
      ...
      <PromptDialog
        ...
        title={t.reopenBtn}
        hint={t.reopenQ}
        confirmLabel={t.reopenBtn}
        cancelLabel={t.cancel}
        danger
        onConfirm={reason => void doReopen(reason)}
      ...
      <ConfirmDialog
        ...
        title={t.resolveBtn}
        hint={t.resolveQ}
        confirmLabel={t.resolveBtn}
        cancelLabel={t.cancel}
        onConfirm={() => void doResolve()}
