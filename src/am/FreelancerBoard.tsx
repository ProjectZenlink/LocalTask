...
export default function AmFreelancerBoard() {
  ...
  const [otherTasks, setOtherTasks] = useState<{ id: string; title: string; commission_override: number | null; freelancer_confirmed_at: string | null }[]>([])
  ...
  const load = useCallback(async () => {
    ...
    const [p, tk, ot, ac, rc, rt] = await Promise.all([
      ...
      supabase.from('tasks').select('id, title, commission_override, freelancer_confirmed_at')
    ...
    setOtherTasks((ot.data ?? []) as { id: string; title: string; commission_override: number | null; freelancer_confirmed_at: string | null }[])
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <Card className="mt-5 p-5">
        ...
        {otherTasks.length === 0 ? (
          ...
          const amt = live ? Number(live.amount) : (ot.commission_override != null ? Number(ot.commission_override) : (rates.get('Other') ?? 0))
          return (
