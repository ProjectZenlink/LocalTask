...
const COPY = {
  zh: {
    ...
    fAll: '全部', fKyc: '已过 KYC', fNoKyc: '未过 KYC', fDone: '有完成任务',
  },
  en: {
    ...
    fAll: 'All', fKyc: 'KYC passed', fNoKyc: 'KYC not passed', fDone: 'Has completed tasks',
  },
}
...
export default function AmFreelancers() {
  ...
  const [flt, setFlt] = useState<'all' | 'kyc' | 'nokyc' | 'done'>('all')
  ...
  async function confirmCi(id: string) {
    ...
    setCiBusy(null)
    if (!e) setCi(prev => ({ ...prev, [id]: 'confirmed' }))
  }
  ...
  const shown = rows
    .filter(r => !needle || [r.display_name, r.full_name, r.email, r.contact_whatsapp, r.contact_telegram]
      .some(v => (v ?? '').toLowerCase().includes(needle)))
    .filter(r => flt === 'all' ? true
      : flt === 'kyc' ? r.kyc_status === 'verified'
      : flt === 'nokyc' ? r.kyc_status !== 'verified'
      : r.completed_tasks > 0)
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-4 flex flex-wrap gap-1.5">
        {([['all', t.fAll], ['kyc', t.fKyc], ['nokyc', t.fNoKyc], ['done', t.fDone]] as const).map(([k, label]) => (
          <button key={k} onClick={() => setFlt(k)}
            className={`rounded-full border px-3 py-1 font-mono text-[11px] uppercase tracking-wider transition ${
              flt === k ? 'border-petrol bg-petrol text-paper' : 'border-hair text-muted hover:border-petrol/40 hover:text-ink'
            }`}>
            {label}
          </button>
        ))}
