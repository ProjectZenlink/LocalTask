...
interface Row { id: string; display_name: string | null; kyc_status: string; open_to_work: boolean; is_suspended: boolean }

const COPY = {
  zh: { title: '我的 Freelancer', sub: '你名下的人才。点进去看八项清单、验收和账号资料。',
        empty: '名下还没有人。去', poolLink: '人才库', empty2: '认领无归属的 freelancer。', accepted: '已验收', suspended: '已暂停' },
  en: { title: 'My freelancers', sub: 'People you manage. Open one for the 8-item checklist, acceptance and account records.',
        empty: 'Nobody yet. Claim unowned freelancers in the', poolLink: 'Pool', empty2: '.', accepted: 'accepted', suspended: 'Paused' },
}
...
export default function AmFreelancers() {
  ...
  const load = useCallback(async () => {
    ...
    const [p, a] = await Promise.all([
      supabase.from('profiles').select('id, display_name, kyc_status, open_to_work, is_suspended')
