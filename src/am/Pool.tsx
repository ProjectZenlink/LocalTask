...
interface Row {
  ...
  open_to_work: boolean; managed_by: string | null; is_rejected: boolean
}

const COPY = {
  zh: { title: '人才库', sub: '全平台的 freelancer。无归属的可以认领到你名下，或驳回不合适的。',
        search: '按名字搜索…', name: '姓名', kyc: 'KYC', open: '接单', owner: '归属',
        mine: '我的', owned: '已归属', claim: '认领', reject: '驳回', rejected: '已驳回', on: '开', off: '关', empty: '暂无数据。',
        rejectQ: '驳回原因(内部记录):', cancel: '取消' },
  en: { title: 'Pool', sub: 'Every freelancer on the platform. Claim unowned ones, or reject the unfit.',
        search: 'Search by name…', name: 'Name', kyc: 'KYC', open: 'Open', owner: 'Owner',
        mine: 'Mine', owned: 'Owned', claim: 'Claim', reject: 'Reject', rejected: 'Rejected', on: 'On', off: 'Off', empty: 'Nothing yet.',
        rejectQ: 'Rejection reason (internal):', cancel: 'Cancel' },
}
...
export default function AmPool() {
  ...
  const load = useCallback(async () => {
    const { data, error: e } = await supabase.from('profiles')
      .select('id, display_name, kyc_status, open_to_work, managed_by, is_rejected')
