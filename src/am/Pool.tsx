...
const COPY = {
  zh: {
    ...
    search: '按名字搜索…', name: '姓名', kyc: 'KYC', open: '接单', load: '活跃/完成', qsa: '质/速/态', strikes: 'Strikes', owner: '归属', act: '',
    mine: '我的', owned: '已归属', none: '—', claim: '认领', reject: '驳回', rejected: '已驳回', assign: '派任务',
    on: '开', off: '关', empty: '暂无数据。', rejectQ: '驳回原因(内部记录):', cancel: '取消',
  ...
  en: {
    ...
    search: 'Search by name…', name: 'Name', kyc: 'KYC', open: 'Open', load: 'Active/Done', qsa: 'Q/S/A', strikes: 'Strikes', owner: 'Owner', act: '',
    mine: 'Mine', owned: 'Owned', none: '—', claim: 'Claim', reject: 'Reject', rejected: 'Rejected', assign: 'Assign task',
    on: 'On', off: 'Off', empty: 'Nothing yet.', rejectQ: 'Rejection reason (internal):', cancel: 'Cancel',
        ...
        {filtered.length === 0 ? (
          ...
          <table className="w-full border-collapse">
            <thead className="border-b border-hair">
              <tr>
                <Th>{t.name}</Th><Th>{t.kyc}</Th><Th>{t.open}</Th>
                <Th>{t.load}</Th><Th>{t.qsa}</Th><Th>{t.strikes}</Th><Th>{t.owner}</Th><Th>{t.act}</Th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(r => (
                <tr key={r.id} className="border-b border-hair last:border-b-0 hover:bg-paper">
                  ...
                  <Td className="font-mono text-xs">
                    {r.managed_by === am?.id ? <span className="text-verified-text">{t.mine}</span>
                      : r.managed_by ? <span className="text-faint">{t.owned}</span> : t.none}
                  </Td>
                  <Td>
