...
export interface SysMsg { k: 'submitted' | 'assigned' | 'support'; am?: string }

export function parseSystemBody(body: string): SysMsg | null {
  try {
    const o = JSON.parse(body) as SysMsg
    if (o && (o.k === 'submitted' || o.k === 'assigned' || o.k === 'support')) return o
  } catch {
    /* 非系统模板:原样显示 */
  }
  return null
...
export function sysText(m: SysMsg, lang: 'zh' | 'en'): string {
  ...
  if (m.k === 'support') {
    return lang === 'zh'
      ? '你已连线 LocalTask Support —— 直接留言,顾问会尽快接手。'
      : 'You are connected with LocalTask Support — leave a message and an advisor will pick this up shortly.'
  }
  const am = m.am ?? ''
  return lang === 'zh'
    ? `已为你分配顾问 ${am},随时可以开始沟通。`
...
/** 接待档署名(m43;与库内 display_name 同源同值) */
export const SUPPORT_NAME = 'LocalTask Support'
