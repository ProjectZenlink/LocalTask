...
// —— OCR:底部区带增强后识别 ——
export async function extractMrzText(cv: CvNS, flat: HTMLCanvasElement): Promise<string> {
  ...
  // 灰度化(增强对比;阈值化交给 Tesseract 自适应)
  const src = cv.imread(zone)
  const gray = new cv.Mat()
  try {
    cv.cvtColor(src, gray, cv.COLOR_RGBA2GRAY)
  } finally { src.delete() }
  // 回写灰度到画布:Tesseract 直接吃 dataURL,灰度图即可
  try {
    const g = gray as unknown as { data: Uint8Array; cols: number; rows: number }
    const img = zctx.createImageData(zone.width, zone.height)
    for (let i = 0; i < g.data.length; i++) {
      img.data[i * 4] = g.data[i]; img.data[i * 4 + 1] = g.data[i]
      img.data[i * 4 + 2] = g.data[i]; img.data[i * 4 + 3] = 255
    }
    zctx.putImageData(img, 0, 0)
  } catch { /* 灰度回写失败则用原彩图 OCR,可用性优先 */ }
  gray.delete()
