...
export default function CommissionReview() {
  ...
  const load = useCallback(async () => {
    const [pRes, dRes] = await Promise.all([
      ...
      supabase.from('platform_acceptances').select(SEL)
        .neq('status', 'pending_admin').order('decided_at', { ascending: false }).limit(20),
    ])
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.id} className="border-b border-hair last:border-b-0 hover:bg-paper">
                  ...
                  <Td><span className="rounded-full border border-hair bg-paper px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-muted">{r.task_type}</span></Td>
