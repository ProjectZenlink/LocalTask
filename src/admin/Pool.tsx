...
import type { PoolRow, AmTransfer } from '../types/database'
...
const COPY = {
  zh: {
    ...
    search: '按名字搜索…', name: '姓名', email: '注册邮箱', kyc: 'KYC', combo: '收款组合', trCard: '归属转移审核', trReasonNone: '(未填理由)', approve: '批准', rejectTr: '驳回', trRejectQ: '驳回备注(可选):',
    load: '负载(活跃·完成)', contact: '联系',
  ...
  en: {
    ...
    search: 'Search by name…', name: 'Name', email: 'Email', kyc: 'KYC', combo: 'Payout combo', trCard: 'Ownership transfer review', trReasonNone: '(no reason)', approve: 'Approve', rejectTr: 'Reject', trRejectQ: 'Rejection note (optional):',
    load: 'Load (active·done)', contact: 'Contact',
...
export default function AdminPool() {
  ...
  async function decideTr(id: string, approve: boolean, note?: string) {
    setTrBusy(true)
    const { error: e } = await supabase.rpc('decide_am_transfer', { p_id: id, p_approve: approve, p_note: note ?? null })
    setTrBusy(false)
  ...
  const load = useCallback(async () => {
    const { data: ptr } = await supabase.from('am_transfers')
      .select('*, fl:profiles!freelancer_id(display_name)').eq('status', 'pending')
      .order('created_at')
    setPendTr((ptr ?? []) as (AmTransfer & { fl: { display_name: string | null } | null })[])
  ...
  return (
    <div>
      ...
      {pendTr.length > 0 && (
        <Card className="mb-4 border-pending-border p-5">
          <p className="mb-3 font-mono text-[11px] uppercase tracking-wider text-pending-text">{t.trCard}</p>
          {pendTr.map(x => (
            <div key={x.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-hair py-2.5 last:border-b-0">
              <div className="min-w-0">
                <p className="text-sm text-ink">
                  {x.fl?.display_name ?? x.freelancer_id.slice(0, 8)}
                  <span className="ml-2 font-mono text-xs text-muted">{amName(x.from_am)} → {amName(x.to_am)}</span>
                </p>
                <p className="mt-0.5 text-xs text-faint">{x.reason ?? t.trReasonNone}</p>
              </div>
              <div className="flex shrink-0 gap-2">
                <Button className="px-3 py-1.5 text-xs" disabled={trBusy} onClick={() => void decideTr(x.id, true)}>{t.approve}</Button>
                <Button variant="ghost" className="px-3 py-1.5 text-xs" disabled={trBusy} onClick={() => setTrRejectId(x.id)}>{t.rejectTr}</Button>
              </div>
            </div>
          ))}
        </Card>
          ...
          <table className="w-full border-collapse">
            <thead className="border-b border-hair">
              <tr>
                ...
                <Th>{t.load}</Th><Th>{t.contact}</Th><Th></Th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(r => (
      ...
      <PromptDialog
        open={trRejectId !== null}
        title={t.rejectTr}
        hint={t.trRejectQ}
        confirmLabel={t.rejectTr}
        cancelLabel="—"
        danger
        onConfirm={note => { const id = trRejectId!; setTrRejectId(null); void decideTr(id, false, note) }}
        onClose={() => setTrRejectId(null)}
      />
    </div>
  )
}
