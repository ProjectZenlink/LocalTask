...
import { usd, dateShort, waLink, tgLink, xLink, signFiles, safeFileName, bjDay } from '../lib/format'
import { PP, PP_KEY, pick, K_PAYOUT_PP_EMAIL } from '../lib/brand'
import DocGrid from '../components/DocGrid'
import FreelancerKycMaterials from './FreelancerKycMaterials'
import { PageHeading, Card, Button, Alert, StatusBadge, SectionTitle, Field, Label, Input, FilePick } from '../components/ui'
...
export default function FreelancerDetail({ amScope = null }: { amScope?: AccountManager | null }) {
  ...
  async function markPaid() {
    ...
    const { error: e } = await supabase.from('profiles')
      .update({ signup_bonus_state: 'paid', bonus_paid_at: new Date().toISOString() })
      .eq('id', p.id)
    setBBusy(false)
    if (e) { setError(friendly(e)); return }
    setBMsg(t.bMarked)
  ...
  async function submitTransfer() {
    ...
    setTrBusy(false)
    if (e) { setError(friendly(e)); setTrDialog(false); return }
  ...
  async function saveEdit() {
    ...
    setEBusy(false)
    if (e) { setEErr(friendly(e)); return }
  ...
  const load = useCallback(async () => {
    if (!id) return
    const [pr, po, ss, co, amls, recs] = await Promise.all([
    ...
    if (pr.error) { setError(friendly(pr.error)); setLoaded(true); return }
  ...
  const coFormCard = (
    <div className="mt-3 rounded-xl border border-petrol/25 bg-paper p-4">
      ...
      <div className="mb-4">
        ...
        <FilePick id="freelancerdetail-files" multiple label="Choose files" empty="No files selected"
          onFiles={fs => setCoFiles(fs)} selectedText={coFiles.length > 0 ? `${coFiles.length} selected` : null} />
  ...
  return (
    <div className="mx-auto max-w-3xl">
            ...
            <Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => {
              if (amScope) {
                void (async () => {
                  ...
                  if (er) { setError(friendly(er)); return }
                  await load()
                })()
              } else {
      ...
      <Card className="mb-5 p-5">
        ...
        {grants.length === 0 ? (
          ...
          <div key={g.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-hair py-3 last:border-b-0">
            ...
            {g.state === 'requested' && (
              <Button className="shrink-0 px-3 py-1.5 text-xs" disabled={gBusy === g.id} onClick={() => void markGrantPaid(g.id)}>
                {gBusy === g.id ? '…' : t.bMark}
              </Button>
            )}
          </div>
      ...
      <FreelancerKycMaterials flId={p.id} amScope={!!amScope} lang={lang} />
