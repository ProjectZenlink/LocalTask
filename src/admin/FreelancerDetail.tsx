...
export default function FreelancerDetail({ amScope = null }: { amScope?: AccountManager | null }) {
  ...
  async function submitTransfer() {
    if (!id || !trTo) return
    setTrBusy(true)
    const { error: e } = await supabase.rpc('request_am_transfer', { p_freelancer: id, p_to_am: trTo, p_reason: trReason.trim() || null })
    setTrBusy(false)
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <Card className="mb-5 p-5">
        ...
        {p.signup_bonus_state === 'requested' && (
          <p className="mt-3 border-t border-hair pt-3">
            <Link to={amScope ? '/am/payouts' : '/admin/payouts'} className="font-mono text-[11px] uppercase tracking-wider text-petrol underline underline-offset-2">{t.bGoPayout}</Link>
          </p>
        )}
      </Card>
