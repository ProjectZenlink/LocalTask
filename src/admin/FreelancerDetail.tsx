...
const COPY = {
  zh: {
    ...
    pause: '暂停', resume: '恢复', block: '封禁', unblock: '解封', claim: '认领到我名下', boardLink: '八项清单',
          ...
          <div key={c.id} className="border-b border-hair py-3 last:border-b-0">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                ...
                {!amScope && <Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => startCoEdit(c)}>{t.coEdit}</Button>}
                {!amScope && <Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => setCoDeleting(c)}>{t.coDelete}</Button>}
              </div>
            </div>
            {openCoId === c.id && (
              coDocs[c.id] === 'loading'
