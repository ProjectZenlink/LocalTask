...
const COPY = {
  zh: {
    title: '账号', sub: '全库已开账号的集中视图。按平台筛选,一处看全:账号 / 密码 / 2FA / 接码手机 / 到期。', catRecords: '账号记录', catCompanies: '公司',
  ...
  en: {
    title: 'Accounts', sub: 'Every opened account in one place. Filter by platform: login / password / 2FA / SMS phone / expiry.', catRecords: 'Account records', catCompanies: 'Companies',
...
export default function AdminAccounts() {
  ...
  const cat = sp.get('cat') === 'companies' ? 'companies' : 'records'
  const { lang } = useLang()
  const t = COPY[lang]
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex gap-5 border-b border-hair">
        {([['records', t.catRecords], ['companies', t.catCompanies]] as const).map(([k, label]) => (
          <button key={k} onClick={() => setSp(k === 'records' ? {} : { cat: k })}
            className={`-mb-px border-b-2 pb-2 text-sm transition ${cat === k ? 'border-petrol font-medium text-ink' : 'border-transparent text-muted hover:text-ink'}`}>
            {label}
          </button>
        ))}
      </div>
