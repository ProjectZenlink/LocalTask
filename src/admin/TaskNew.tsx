...
import type { Client, PoolRow } from '../types/database'
...
const COPY = {
  zh: {
    title: '新建任务', sub: '客户和验收标准是必填 —— 验收标准是之后审核和裁决的唯一依据。金额一律美元,freelancer 用自己设置的钱包收款。',
    ...
    assignee: '直接发 Offer 给(可选)', assigneeNone: '先不指派(创建后从任务详情的池子里挑)',
    active: '活跃', create: '创建任务', creating: '创建中…',
    vClient: '请选择客户。', vTitle: '请填任务标题。', vCriteria: '验收标准必填。', vAmount: '金额需大于 0。',
    offerFail: '任务已创建,但直接发 Offer 失败:',
  },
  en: {
    title: 'New task', sub: 'Client and acceptance criteria are required — the criteria are the only basis for review and disputes. Amounts are in USD; freelancers receive to their own wallet.',
    ...
    assignee: 'Send offer directly to (optional)', assigneeNone: "Don't assign yet (pick from the pool on the task page)",
    active: 'active', create: 'Create task', creating: 'Creating…',
          ...
          <select value={assignee} onChange={e => setAssignee(e.target.value)} className={selectCls}>
            ...
            {pool.map(p => (
              <option key={p.id} value={p.id}>
                ...
                {p.payout_network && p.payout_token ? ` · ${payoutLabel(p.payout_network, p.payout_token)}` : ''}
