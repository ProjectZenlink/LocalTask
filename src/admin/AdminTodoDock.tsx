...
export default function AdminTodoDock() {
  ...
  if (folded) {
    return (
      <button onClick={toggle}
        className={`fixed bottom-5  z-30 flex items-center gap-2 rounded-full border border-hair bg-surface px-3.5 py-2 font-mono text-[11px] uppercase tracking-wider text-muted shadow-[0_8px_28px_rgba(26,32,30,0.16)] transition hover:text-ink transition-[right] duration-300 ${dockOpen ? 'right-4 lg:right-[456px]' : 'right-5'}`}>
        {t.pill}
        {rows.length > 0 && (
  ...
  return (
    <div className={`fixed bottom-5 z-30 w-72 rounded-2xl border border-hair bg-surface p-4 shadow-[0_16px_48px_rgba(26,32,30,0.18)] transition-[right] duration-300 ${dockOpen ? 'right-4 lg:right-[456px]' : 'right-5'}`}>
