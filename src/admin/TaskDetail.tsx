...
import { payoutLabel, PLATFORMS } from '../types/database'
...
const COPY = {
  zh: {
    ...
    tx: '交易',
    back: '← 任务列表', tags: '标签', dlgCancel: '取消', copyLink: '复制客户链接', copied: '已复制 ✓', edit: '编辑任务', save: '保存修改', saving: '保存中…', cancelEdit: '取消编辑', addFiles: '追加附件(可多选)', tagHint: '标签(回车或逗号添加)', rate: '三维评分', quality: '质量', speed: '速度', attitude: '态度',
    ...
    commission: '提成', byRate: '按费率表', override1: '(单笔覆盖)', editCom: '改提成',
  ...
  en: {
    ...
    tx: 'Transaction',
    back: '← All tasks', tags: 'Tags', dlgCancel: 'Cancel', copyLink: 'Copy client link', copied: 'Copied ✓', edit: 'Edit task', save: 'Save changes', saving: 'Saving…', cancelEdit: 'Cancel editing', addFiles: 'Add attachments (multiple)', tagHint: 'Tags (Enter or comma to add)', rate: 'Rating', quality: 'Quality', speed: 'Speed', attitude: 'Attitude',
    ...
    commission: 'Commission', byRate: 'per rate table', override1: '(override)', editCom: 'Edit commission',
...
export default function AdminTaskDetail({ amScope = null }: { amScope?: AccountManager | null }) {
  ...
  const doCancel = (reason: string) => {
    ...
    void run(async () => {
      ...
      return supabase.from('tasks').update({ status: 'cancelled', cancelled_reason: reason }).eq('id', task.id)
    })
  }
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-5 flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 flex-col items-end gap-2">
          ...
          <div className="flex gap-2">
            ...
            {task.status === 'unassigned' && !editing && (
      ...
      <Card className="mb-5 p-5">
        ...
        {task.task_type && (
          <KV k={t.typeK}>
            <span className="rounded-full border border-hair bg-paper px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-muted">{task.task_type}</span>
          </KV>
        )}
