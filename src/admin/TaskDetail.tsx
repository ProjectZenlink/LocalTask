...
import { Card, Button, Alert, Label, Input, Textarea, Linkified, SectionTitle, FilePick } from '../components/ui'
...
const COPY = {
  zh: {
    ...
    match: '可派的 Freelancer(基础 KYC 已认证)',
    matchEmpty: '暂无可派的人——还没有已认证的 freelancer。',
    ...
    vNote: '退回必须填写审核意见。',
    payTitle: '放款', payReq: 'freelancer 已在钱包申请提现', payHint1: '把下面的地址和金额发给客户打款:', payHint2: '确认客户已打款后,点击标记:',
    markPaid: '标记已付款', paid: '已标记付款。',
    tx: '交易',
  ...
  en: {
    ...
    matchEmpty: 'No one assignable yet — no verified freelancers.',
    ...
    payTitle: 'Payment', payReq: 'Freelancer requested payout from their wallet', payHint1: 'Send this address and amount to the client:', payHint2: 'Once the client has paid, mark it here:',
    markPaid: 'Mark as paid', paid: 'Marked paid.',
    tx: 'Transaction',
...
export default function AdminTaskDetail({ amScope = null }: { amScope?: AccountManager | null }) {
  ...
  const load = useCallback(async () => {
    ...
    if (tRes.error) { setError(friendly(tRes.error)); setLoaded(true); return }
  ...
  async function run(fn: () => Promise<{ error: { message: string } | null } | void>) {
    ...
    try {
      const res = await fn()
      if (res && res.error) setError(friendly(res.error))
    } catch (e) {
      setError(friendly(e))
    } finally {
      setBusy(false)
  ...
  return (
    <Card className="mb-5 border-petrol/25 p-5">
      ...
      <div className="mb-5">
        ...
        <FilePick id="taskdetail-files" multiple label="Choose files" empty="No files selected"
          onFiles={fs => setFiles(fs)} selectedText={files.length > 0 ? `${files.length} selected` : null} />
