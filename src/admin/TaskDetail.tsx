...
import { money, dateShort, dateTimeShort, timeLeft, txUrl, shortHash, signTaskFiles, fileNameFromPath, isImagePath } from '../lib/format'
...
type TaskRow = Task & { client: { company_name: string } | null }

const COPY = {
  zh: {
    notFound: '任务不存在', client: '客户', amount: '金额', deadline: '截止', created: '创建',
    assignee: '接单人', payout: '收款地址(快照)', none: '—',
    brief: '任务说明', files: '附件', criteria: '验收标准',
    match: '匹配的 Freelancer(已认证 · 开着接单 · 钱包组合一致)',
    matchEmpty: '暂无匹配的人。可能没人开着接单,或钱包组合对不上。',
    offerNote: 'Offer 备注(可选,freelancer 可见)', expiry: '过期时长(小时)', send: '发 Offer', sending: '发送中…',
    active: '活跃', done: '完成', strikes: 'Strikes',
  ...
  en: {
    notFound: 'Task not found', client: 'Client', amount: 'Amount', deadline: 'Due', created: 'Created',
    assignee: 'Assignee', payout: 'Payout address (snapshot)', none: '—',
    brief: 'Brief', files: 'Attachments', criteria: 'Acceptance criteria',
    match: 'Matching freelancers (verified · open to work · same wallet combo)',
    matchEmpty: 'No matches. Nobody is open to work, or wallet combos differ.',
    offerNote: 'Offer note (optional, visible to freelancer)', expiry: 'Expires in (hours)', send: 'Send offer', sending: 'Sending…',
    active: 'Active', done: 'Done', strikes: 'Strikes',
...
export default function AdminTaskDetail() {
  ...
  const load = useCallback(async () => {
    ...
    const [tRes, oRes, sRes, rRes] = await Promise.all([
      supabase.from('tasks').select('*, client:clients(company_name)').eq('id', id).maybeSingle(),
    ...
    if (tk) {
      if (tk.status === 'unassigned') {
        const { data: p } = await supabase.from('freelancer_pool').select('*')
          ...
          .eq('payout_network', tk.payout_network).eq('payout_token', tk.payout_token)
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <Card className="mb-5 p-5">
        <KV k={t.client}>{task.client?.company_name ?? t.none}</KV>
        <KV k={t.amount}><span className="font-mono">{money(task.amount, task.payout_token)} · {payoutLabel(task.payout_network, task.payout_token)}</span></KV>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead className="border-b border-hair">
                  <tr><Th>Freelancer</Th><Th>{t.active}</Th><Th>{t.done}</Th><Th>Q / S / A</Th><Th>{t.strikes}</Th><Th>WhatsApp / TG</Th><Th></Th></tr>
                </thead>
                <tbody>
                  {pool.map(p => (
        ...
        <Card className="mb-5 border-verified-border bg-verified-bg p-5">
          ...
          {!task.paid_at ? (
            <>
              ...
              <p className="mt-1 font-mono text-xs text-ink">{money(task.amount, task.payout_token)} · {payoutLabel(task.payout_network, task.payout_token)}</p>
