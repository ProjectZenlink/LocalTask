...
export default function AdminKyc() {
  ...
  const [amNames, setAmNames] = useState<Record<string, string>>({})
  ...
  const loadQueue = useCallback(async () => {
    ...
    if (e1) { setError(friendly(e1)); return }
    const rows = subs ?? []
    const ids = [...new Set(rows.map(s => s.user_id))]
    const [profRes, ssnRes, amsRes] = await Promise.all([
    ...
    setAmNames(Object.fromEntries(((amsRes.data ?? []) as { id: string; name: string }[]).map(a => [a.id, a.name])))
    setQueue(rows.map(s => {
  ...
  async function review(sub: PendingSub, approve: boolean, reason: string | null = null) {
    ...
    if (e1) { setBusyId(null); setError(friendly(e1)); return }
