...
type Row = Task & { client: { company_name: string } | null; assigned: { display_name: string | null } | null }
...
export default function AdminTasks() {
  ...
  useEffect(() => {
    supabase
      .from('tasks')
      .select('*, client:clients(company_name), assigned:profiles!tasks_assigned_freelancer_fkey(display_name)')
            ...
            <tbody>
              {filtered.map(r => (
                <tr key={r.id} className="border-b border-hair last:border-b-0 hover:bg-paper">
                  ...
                  <Td className="text-muted">{r.client?.company_name ?? t.none}</Td>
                  <Td className="whitespace-nowrap font-mono text-xs">{money(r.amount, r.payout_token)}</Td>
