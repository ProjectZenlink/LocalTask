...
type Row = Task & { client: { company_name: string; contact_name: string | null } | null; assigned: { display_name: string | null } | null }

const COPY = {
  zh: {
    title: '任务管理', sub: '建任务、发 Offer、审交付、标记放款、打分 —— 全流程都在这里。',
    newTask: '＋ 新建任务', search: '按标题搜索…', all: '全部',
    thTitle: '任务', thClient: '客户', thAmount: '金额', thAssigned: '接单人', thDeadline: '截止', thStatus: '状态',
  ...
  en: {
    ...
    newTask: '＋ New task', search: 'Search by title…', all: 'All',
    thTitle: 'Task', thClient: 'Client', thAmount: 'Amount', thAssigned: 'Assignee', thDeadline: 'Due', thStatus: 'Status',
...
export default function AdminTasks() {
  ...
  useEffect(() => {
    supabase
      .from('tasks')
      .select('*, client:clients(company_name, contact_name), assigned:profiles!tasks_assigned_freelancer_fkey(display_name)')
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-4">
        ...
        <Link to="/admin/tasks/new"><Button>{t.newTask}</Button></Link>
      </div>
        ...
        {!loaded ? (
          ...
          <table className="w-full border-collapse">
            <thead className="border-b border-hair">
              <tr>
                <Th>{t.thTitle}</Th><Th>{t.thClient}</Th><Th>{t.thAmount}</Th>
              ...
              {filtered.map(r => (
                <tr key={r.id} className="border-b border-hair last:border-b-0 hover:bg-paper">
                  <Td className="max-w-[26rem]">
                    <Link to={`/admin/tasks/${r.id}`} className="block truncate text-ink hover:text-petrol">{r.title}</Link>
                  </Td>
                  <Td className="text-muted">{r.client ? r.client.company_name + (r.client.contact_name ? ' · ' + r.client.contact_name : '') : t.none}</Td>
