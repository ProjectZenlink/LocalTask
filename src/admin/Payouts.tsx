...
const COPY = {
  zh: {
    ...
    chipPending: '待打款', chipPaid: '待 FREELANCER 确认', chipDone: '已确认', chipRejected: '已驳回',
  ...
  en: {
    ...
    chipPending: 'To pay', chipPaid: 'AWAITING FREELANCER', chipDone: 'Confirmed', chipRejected: 'Rejected',
...
export default function AdminPayouts() {
  ...
  const chip = (s: Row['status']) => {
    const map = {
      ...
      paid_pending_confirm: ['border-pending-border bg-pending-bg text-pending-text', `● ${t.chipPaid}`],
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex flex-wrap gap-2.5">
        {([['pending', t.tabPending, pending.length], ['paid', t.tabPaid, paid.length], ['hist', t.tabHist, hist.length]] as const).map(([k, label, n]) => (
