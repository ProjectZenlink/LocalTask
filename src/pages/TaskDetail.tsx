...
export default function TaskDetail() {
  ...
  const load = useCallback(async () => {
    ...
    const t = (tRes.data ?? null) as TaskRow | null
    setTask(t)
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <Link to="/tasks" className="mb-3 inline-block font-mono text-[11px] uppercase tracking-wider text-faint transition hover:text-ink">← Tasks</Link>
      ...
      {task.am && (
        <Card className="mb-5 p-5">
          <SectionTitle>Your account manager</SectionTitle>
          ...
          <p className="mt-2 text-xs text-faint">Questions about the brief, deadline, or payment? Message them directly.</p>
        </Card>
      )}
          ...
          {!task.paid_at ? (
            <>
              <p className="text-sm leading-relaxed text-ink">
                ...
                {task.payout_method === 'paypal' ? 'directly to your PayPal:' : 'directly to your wallet:'}
              </p>
            ...
            <>
              <p className="text-sm leading-relaxed text-ink">
                ...
                {task.tx_hash && (task.payout_method === 'paypal' || !task.payout_network ? (
                  <> — reference <span className="font-mono text-ink">{shortHash(task.tx_hash)}</span></>
                ) : (
                  <> — transaction{' '}
              ...
              <p className="mt-3 text-xs text-muted">
                {task.payout_method === 'paypal' ? 'Check your PayPal.' : 'Check your wallet.'} If the funds arrived, confirm below to close the task.
      ...
      {task.status === 'completed' && (
        <Card className="mb-5 border-verified-border bg-verified-bg p-5">
          ...
          <p className="text-sm leading-relaxed text-ink">
            Paid {task.payout_method === 'paypal' ? `${usd(task.amount)} via PayPal` : taskMoney(task.amount, task.payout_token)} · {lt(task.amount)} settled
            {task.tx_hash && (task.payout_method === 'paypal' || !task.payout_network ? (
              <> · ref <span className="font-mono">{shortHash(task.tx_hash)}</span></>
            ) : (
              <> · <a href={txUrl(task.payout_network, task.tx_hash)} target="_blank" rel="noreferrer"
                className="font-mono text-petrol underline underline-offset-2">{shortHash(task.tx_hash)}</a></>
            ))}
