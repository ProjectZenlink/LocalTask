...
import {
  money, dateShort, dateTimeShort, timeLeft, txUrl, shortHash,
    ...
    <div className="mx-auto max-w-2xl">
      <div className="mb-6 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-2 font-mono text-xs text-faint">
            {money(task.amount, task.payout_token)} · {payoutLabel(task.payout_network, task.payout_token)}
          ...
          {!task.paid_at ? (
            <>
              <p className="text-sm leading-relaxed text-ink">
                Your work passed review. The client has been asked to send{' '}
                <span className="font-mono">{money(task.amount, task.payout_token)}</span> directly to your wallet:
              </p>
      ...
      {task.status === 'completed' && (
        <Card className="mb-5 border-verified-border bg-verified-bg p-5">
          ...
          <p className="text-sm leading-relaxed text-ink">
            Paid {money(task.amount, task.payout_token)}
            {task.tx_hash && (
              <> · <a href={txUrl(task.payout_network, task.tx_hash)} target="_blank" rel="noreferrer"
                className="font-mono text-petrol underline underline-offset-2">{shortHash(task.tx_hash)}</a></>
            )}
