...
import { PageHeading, Card, Button, Alert, SectionTitle, TaskBadge, Textarea, Label, Linkified, FilePick } from '../components/ui'
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-6 flex items-start justify-between gap-3">
        ...
        <TaskBadge status={task.status} lang="en" />
      </div>
      ...
      {mine && task.status === 'in_progress' && (
        <Card className="mb-5 p-5">
          ...
          <div className="mb-4">
            ...
            <FilePick id="task-files-input" multiple label="Choose files" empty="No files selected"
              onFiles={fs => setFiles(fs)}
              selectedText={files.length > 0 ? `${files.length} selected` : null} />
            {files.length > 0 && (
