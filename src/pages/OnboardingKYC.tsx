...
/** v85.9:护照优先。默认「护照单页」模式(存 id_front 槽,零迁移);
 *  右上角一键切回证件正/反面模式。地址证明与 SSN 两种模式一致。 */
const ID_DOCS = KYC_DOCS.filter(d => d.key !== 'selfie_handheld')
...
const COPY = {
  en: {
    ...
    modePassport: "I don't have a passport — use my ID instead",
    modeId: 'I have a passport — use it instead',
...
export default function OnboardingKYC() {
  ...
  const t = COPY.en
  const [mode, setMode] = useState<'passport' | 'id'>('passport')
  ...
  function switchMode() {
    ...
    setMode(m => (m === 'passport' ? 'id' : 'passport'))
    ...
    <div className="mx-auto max-w-lg">
      <div className="flex items-start justify-between gap-4">
        ...
        <button type="button" onClick={switchMode}
          className="mt-1 shrink-0 text-right text-xs text-petrol underline underline-offset-2 transition hover:opacity-80">
          {mode === 'passport' ? t.modePassport : t.modeId}
        </button>
      </div>
      ...
      {DOCS.map(d => {
        ...
        return (
          <div key={`${mode}-${d.key}`} className="mb-5 rounded-xl border border-hair bg-surface p-4">
