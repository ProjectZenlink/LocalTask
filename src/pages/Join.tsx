...
const COPY = {
  en: {
    ...
    errEmail: 'Please enter a valid email address.', wa: 'WhatsApp number', tg: 'Telegram (optional)',
  ...
  zh: {
    ...
    errEmail: '请输入有效的邮箱地址。', wa: 'WhatsApp 号码', tg: 'Telegram(选填)',
    ccLabel: '国家码',
    privacy: '这些信息只用于回复你。',
    privacyLink: '隐私政策',
    cta: '开始对话', busy: '连接中…',
...
function JoinInner() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => setPhase(p => (p === 'probe' ? 'form' : p)), 1200)
    ...
    try {
      ...
      if (!sess.session) {
        ...
        if (aerr) {
          ...
          setError(friendly(aerr)); setBusy(false); return
        }
      }
      ...
      if (ferr || !r?.ok) {
        // v68.1:非 2xx 时 data 为空,真实错误码在 FunctionsHttpError.context 里 —— 解包映射成人话
        let code = r?.error ?? ''
        if (!code && ferr && typeof ferr === 'object' && 'context' in ferr) {
          try {
            const eb = await (ferr as { context: Response }).context.json() as { error?: string } | null
            code = eb?.error ?? ''
          } catch { /* 保底走通用文案 */ }
        }
        if (code === 'signed_in') { setPhase('member'); setBusy(false); return }
        captchaRef.current?.reset(); setCaptchaToken(null)
        setError(code === 'captcha' ? t.errCaptcha : code === 'bad_email' ? t.errEmail : code || ferr?.message || t.errGeneric)
        setBusy(false)
        return
      }
      ...
      <main className="flex flex-1 items-start justify-center px-4 pb-16 pt-4 sm:pt-8">
        ...
        {phase === 'form' && (
          <div className="w-full max-w-md rounded-2xl border border-hair bg-white p-8 shadow-sm sm:p-10">
            ...
            <div className="mt-4">
              <Label>{t.email}</Label>
              <Input type="email" autoComplete="email" placeholder={t.emailPh}
