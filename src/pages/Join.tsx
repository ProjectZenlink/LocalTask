...
import { COUNTRY_CODES, normalizeWa, waLooksValid } from '../lib/leads'
...
const COPY = {
  en: {
    ...
    advisor: 'Advisor',
    poolT: 'You are in the queue',
    poolB: 'All advisors are momentarily offline. Keep this page open — the moment one comes online, your chat starts right here.',
    ...
    submittedNote: 'Request submitted.',
  },
  zh: {
    title: '和本地顾问聊聊。',
    ...
    advisor: '顾问',
    poolT: '正在排队',
    poolB: '顾问暂时都不在线。保持本页打开 —— 一旦有顾问上线,对话会立刻在这里开始。',
    convT: '你的账号已就绪',
...
function JoinInner() {
  ...
  useEffect(() => {
    ...
    const interval = lead?.assigned_am ? 30_000 : 12_000
  ...
  // 有归属即开线(幂等找旧建新);认领改派后 user_id 变化会自动换到新线
  useEffect(() => {
    const amUser = lead?.am?.user_id
    if (phase !== 'ready' || !amUser) { setConvId(null); return }
    let alive = true
    void supabase.rpc('open_conversation', { p_other: amUser }).then(({ data }) => {
      if (alive && typeof data === 'string') setConvId(data)
    })
    return () => { alive = false }
  }, [phase, lead?.am?.user_id])
      ...
      <main className="flex flex-1 items-start justify-center px-4 pb-16 pt-4 sm:pt-8">
        ...
        {phase === 'ready' && lead && (
          <div className="w-full max-w-2xl">
            ...
            {!lead.assigned_am && !converted && (
              <div className="rounded-2xl border border-hair bg-white p-8 text-center shadow-sm">
                <span className="mx-auto flex h-10 w-10 items-center justify-center">
                  <span className="h-2.5 w-2.5 animate-pulse rounded-full bg-petrol motion-reduce:animate-none" />
                </span>
                <p className="mt-3 font-display text-xl font-medium tracking-tight text-ink">{t.poolT}</p>
                <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-muted">{t.poolB}</p>
                <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.2em] text-faint">{t.submittedNote}</p>
              </div>
            )}
              ...
              <div className="overflow-hidden rounded-2xl border border-hair bg-white shadow-sm">
                <div className="flex items-center gap-3 border-b border-hair bg-surface px-4 py-3">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-petrol font-display text-sm font-medium text-paper">
                    {(amName ?? '?').trim().charAt(0).toUpperCase() || '?'}
                  </span>
                  <span className="min-w-0">
                    <span className="block truncate text-sm font-medium text-ink">{amName ?? '—'}</span>
                    <span className="block font-mono text-[9.5px] uppercase tracking-[0.18em] text-faint">{t.advisor}</span>
                  </span>
                </div>
                <div className="h-[62vh] min-h-[24rem]">
                  {convId && user ? (
                    <LeadChat conversationId={convId} meId={user.id} lang={lang} readOnly={converted} />
                  ) : (
