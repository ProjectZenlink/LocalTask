...
const COPY = {
  ...
  zh: {
    ...
    ciAll: '签到奖励已全部解锁 🎉',
    availHead: '可提现', availEmpty: '完成任务、保持签到。',
...
export default function Wallet() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {/* 可提现 · Hero 余额卡(v52) */}
      <Card className="mb-5 overflow-hidden border-transparent bg-petrol p-0 text-paper shadow-[0_16px_40px_rgba(36,75,77,0.28)]">
        <div className="p-5">
          <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-paper/55">{t.availHead}</p>
          <p className="mt-2.5 font-mono text-4xl tracking-tight">{usd(animTotal)}</p>
          <p className="mt-1 font-mono text-[11px] text-paper/50">{lt(availTotal)}</p>
          {availTasks.length === 0 && availGrants.length === 0 && !bonusAvail && !bonusGate && !bonusPendingKyc ? (
            <p className="mt-3.5 text-sm text-paper/60">{t.availEmpty}</p>
          ) : (
            <div className="mt-4 space-y-1.5 border-t border-paper/15 pt-3.5">
              ...
              {bonusPendingKyc && <p className="font-mono text-xs text-paper/60">{t.bonusPendingKyc}</p>}
      ...
      {openReqs.length > 0 && (
        <Card className="mb-5 p-5">
          ...
          <div className="mt-3 space-y-4">
            {openReqs.map(r => (
              <div key={r.id} className={`rounded-xl border border-hair border-l-2 bg-white p-3.5 ${r.status === 'pending' ? 'border-l-pending-text/60' : 'border-l-petrol/60'}`}>
