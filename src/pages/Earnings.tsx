...
const COPY = {
  en: {
    ...
    ciTitle: 'Daily check-in', ciBtn: 'Check in', ciDone: (n: number) => `Checked in today · ${n} day streak`,
  ...
  zh: {
    ...
    ciTitle: '每日签到', ciBtn: '签到', ciDone: (n: number) => `今日已签到 · 连续 ${n} 天`,
...
export default function Wallet() {
  ...
  const loadExtras = useCallback(async () => {
    ...
    const [g, c] = await Promise.all([
      ...
      supabase.from('checkins').select('day').eq('user_id', user.id).order('day', { ascending: false }).limit(40),
    ])
    setGrants((g.data ?? []) as BonusGrant[])
    setDays(((c.data ?? []) as { day: string }[]).map(x => x.day))
  }, [user])
  ...
  const checkedToday = daySet.has(bjToday)
  let streak = 0
  {
    const start = new Date(checkedToday ? bjToday : new Date(Date.now() + 8 * 3600_000 - 86400_000).toISOString().slice(0, 10))
    for (let i = 0; ; i++) {
      const d = new Date(start.getTime() - i * 86400_000).toISOString().slice(0, 10)
      if (daySet.has(d)) streak++
      else break
    }
    if (!checkedToday && streak === 0) streak = 0
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-ink">
              {checkedToday ? t.ciDone(streak) : t.ciStreak(streak)}
            </p>
