...
const COPY = {
  en: {
    ...
    steps: [
      ...
      { k: 'Unlock', cls: 'text-petrol', d: 'Work approved → LT Coins unlock and the client is asked to pay your wallet or payment account directly.' },
  ...
  zh: {
    ...
    sub: 'LT Coins 记录平台欠你的钱，1 LT Coin = 1 美元。真钱由客户直接打到你的钱包或收款账户。',
    ...
    steps: [
      ...
      { k: '解锁', cls: 'text-petrol', d: '任务审核通过后 LT Coins 解锁，客户会被要求直接付款到你的钱包或收款账户。' },
...
/** Wallet(原 Coins):LT 是记账展示层,真钱由客户直付你的钱包/收款账户。 */
export default function Wallet() {
  ...
  const isPaypal = profile?.payout_method === PP_KEY
  const payoutSet = isPaypal ? !!pick(profile, K_PAYOUT_PP_EMAIL) : !!profile?.payout_address
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            {payoutSet ? (
              isPaypal ? (
                <>
                  <p className="mt-1 font-display text-sm font-medium tracking-tight text-ink">{PP}</p>
                  <p className="mt-0.5 break-all font-mono text-xs text-muted">{pick(profile, K_PAYOUT_PP_EMAIL)}</p>
                </>
              ) : (
                <>
          ...
          <div key={x.id} className="flex items-center justify-between gap-3 border-b border-hair py-3 last:border-b-0">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 font-mono text-xs text-faint">
                {dateShort(x.freelancer_confirmed_at)} · {x.payout_method === PP_KEY ? `${PP} · ${usd(x.amount)}` : taskMoney(x.amount, x.payout_token)}
                {x.tx_hash && (x.payout_method === PP_KEY || !x.payout_network ? (
