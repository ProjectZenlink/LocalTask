...
import { PP, PP_KEY, USDT, USDC, ETH_W, K_PP_EMAIL, pick } from '../lib/brand'
...
const COPY = {
  en: {
    ...
    payIntro1: `Keep several ways to get paid — crypto wallets and ${PP} can coexist. The one marked`, snapWhat: 'What is snapshot protection?', pcrHintShort: 'Changes go through a quick review by your AM or an admin.',
    ...
    mCrypto: 'Crypto wallet', mCryptoHint: `${USDT} · ${USDC} · ${ETH_W}`, mPaypal: PP, mPaypalHint: 'Personal email',
    netLabel: 'Network & token', addrLabel: 'Wallet address', addrPh: 'Select an option first',
    ppLabel: `${PP} email`,
    ppHint: `Use a ${PP} account in your own legal name — it must match your KYC identity.`,
    errPaypal: `That doesn't look like a valid ${PP} email.`,
...
export default function Profile() {
  ...
  const [payMethod, setPayMethod] = useState<string>('crypto')
  ...
  async function addMethod() {
    ...
    if (payMethod === PP_KEY) {
      ...
      payload = { user_id: user.id, method: PP_KEY, [K_PP_EMAIL]: em }
    } else {
    ...
    if (wasEmpty && data) {
      ...
      if (e2) { setBusy(null); setError(friendly(e2)); await loadMethods(); return }
      await refresh()
    }
    setBusy(null)
    setSaved('wallet')
  ...
  async function submitPcr() {
    ...
    setPcrBusy(false)
    if (e) { setError(friendly(e)); setPcrDialog(false); return }
    setPcrDialog(false)
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <Card className="mb-5 p-5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setSnapOpen(v => !v)}
            className="mb-4 font-mono text-[10px] uppercase tracking-wider text-faint transition hover:text-petrol">
            {t.snapWhat} {snapOpen ? '−' : '+'}
          </button>
        </div>
        <div className={`fold-wrap ${snapOpen ? 'open' : ''}`}><div className="fold-inner">
          <p className="mb-3 text-xs leading-relaxed text-muted">
            {t.payIntro1}<span className="font-mono text-[10px]"> DEFAULT </span>{t.payIntro2} {t.lockNote}
          </p>
        </div></div>
              ...
              <div key={m.id} className={`flex flex-wrap items-center justify-between gap-2 rounded-xl border px-3 py-2.5 ${m.is_default ? 'border-petrol bg-petrol/5' : 'border-hair'}`}>
                <div className="min-w-0">
                  <p className="flex items-center gap-2 text-sm font-medium text-ink">
                    {m.method === PP_KEY ? PP : (m.network && m.token ? payoutLabel(m.network, m.token) : t.crypto)}
                    {m.is_default && <span className="rounded-full bg-petrol px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-paper">{t.defaultChip}</span>}
                  </p>
                  <p className="mt-0.5 break-all font-mono text-xs text-muted">{m.method === PP_KEY ? pick(m, K_PP_EMAIL) : m.address}</p>
                </div>
          ...
          <div className="rounded-xl border border-petrol/25 bg-paper p-4">
            <div className="mb-4 grid grid-cols-2 gap-2">
              {([
                ...
                { key: PP_KEY, label: t.mPaypal, hint: t.mPaypalHint },
              ]).map(mm => (
      ...
      <Card className="mb-5 p-5">
        ...
        {pcr ? (
          ...
          <>
            <p className="mt-3 text-xs text-faint">{t.pcrHintShort}</p>
      ...
      {pcrDialog && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-ink/30 p-4" onClick={() => setPcrDialog(false)}>
          <div className="w-full max-w-md rounded-2xl border border-hair bg-white p-6 shadow-sm" onClick={e => e.stopPropagation()}>
            ...
            <p className="mb-4 text-xs leading-relaxed text-muted">{t.pcrHint} {t.pcrDlgHint}</p>
