...
const COPY = {
  en: {
    ...
    kycVerified: 'You are verified and can receive offers.',
    kycPending: 'Documents under review — usually 1–2 business days. Need it faster? Contact support below.',
    kycNeeded: 'Verification is required before you can receive offers.',
    kycSupport: 'Contact support on Telegram',
    resubmit: 'Resubmit documents', start: 'Start verification',
    openHead: 'Open to work',
    openOn: 'Account managers can send you task offers.',
    openOffVerified: 'Turn this on to start receiving offers.',
    openLocked: 'Unlocks after your identity is verified.',
    saving: 'Saving…', turnOff: 'Turn off', turnOn: 'Turn on',
    ...
    payIntro2: 'is snapshotted into each task the moment you accept it.',
    payEmpty: "No payout method yet — you can't accept offers until you add one.",
    ...
    lockNote: 'While a task is active, switching the default is locked — payment details were snapshotted when you accepted. Payments always go to the default marked above.',
    contactHead: 'Contact for your account manager',
    contactSub: 'Offers and coordination happen over WhatsApp or Telegram — add at least one.',
    whatsapp: 'WhatsApp (with country code)', telegram: 'Telegram username', x: 'X (Twitter) username',
    saveContact: 'Save contact',
    accountHead: 'Account',
    displayName: 'Display name', fullName: 'Legal full name', street: 'Street address',
  ...
  zh: {
    ...
    kycHead: '身份验证',
    kycVerified: '你已通过验证，可以接收邀约。',
    kycPending: '材料审核中，通常 1–2 个工作日。想加急？用下方链接联系支持。',
    kycNeeded: '需要先完成身份验证才能接收邀约。',
    kycSupport: '在 Telegram 联系支持',
    resubmit: '重新提交材料', start: '开始验证',
    openHead: '接单状态',
    openOn: '账户经理可以给你发任务邀约。',
    openOffVerified: '打开后开始接收邀约。',
    openLocked: '身份验证通过后解锁。',
    saving: '保存中…', turnOff: '关闭', turnOn: '打开',
    suspended: '你的账号已被平台暂停，不会收到邀约。',
    saved: '已保存。',
    payHead: '收款方式',
    payIntro1: '可以同时保存多种收款方式——加密钱包和 PayPal 可并存。标记为',
    payIntro2: '的会在你接受任务那一刻被快照进任务。',
    payEmpty: '还没有收款方式。添加一个之前无法接受邀约。',
    ...
    lockNote: '有任务进行中时，默认收款方式会锁定——收款信息在你接受任务时已快照。付款始终打到上面标记为默认的那个。',
    contactHead: '给账户经理的联系方式',
    contactSub: '邀约和沟通都在 WhatsApp/Telegram 上进行，至少填一个。',
    whatsapp: 'WhatsApp（含国家区号）', telegram: 'Telegram 用户名', x: 'X（Twitter）用户名',
    saveContact: '保存联系方式',
    accountHead: '账户资料',
    displayName: '昵称', fullName: '法定全名', street: '街道地址',
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <Card className="mb-5 p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            <SectionTitle>{t.openHead}</SectionTitle>
            <p className="text-sm text-muted">
          ...
          <Button
            ...
            onClick={toggleOpen}
          >
            {busy === 'availability' ? t.saving : profile.open_to_work ? t.turnOff : t.turnOn}
          </Button>
        </div>
        {profile.is_suspended && (
          <p className="mt-3 text-sm text-danger-text">{t.suspended}</p>
        )}
        {saved === 'availability' && <p className="mt-3 text-sm text-verified-text">{t.saved}</p>}
      </Card>
      ...
      <Card className="mb-5 p-5">
        ...
        <p className="mb-4 text-sm text-muted">{t.contactSub}</p>
