...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    setPayMethod(profile.payout_method ?? 'crypto')
  ...
  async function saveWallet() {
    if (payMethod === 'paypal') {
      ...
      await save('wallet', { payout_method: 'paypal', payout_paypal_email: em })
      return
    }
    ...
    await save('wallet', { payout_method: 'crypto', payout_network: opt.network, payout_token: opt.token, payout_address: addr })
  }
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <Card className="mb-5 p-5">
        ...
        <p className="mb-4 text-sm text-muted">
          Clients pay you directly — the platform never holds your money. Pick one method per account.
        </p>
        <div className="mb-4 grid grid-cols-2 gap-2">
          {([
            ...
            <button key={m.key} type="button" onClick={() => setPayMethod(m.key)}
              className={`rounded-xl border px-3 py-3 text-left transition ${payMethod === m.key ? 'border-petrol bg-petrol/5' : 'border-hair hover:border-petrol/40'}`}>
              <p className={`font-display text-sm font-medium ${payMethod === m.key ? 'text-petrol' : 'text-ink'}`}>{m.label}</p>
        ...
        {payMethod === 'crypto' ? (
          ...
          <div className="mb-4">
            ...
            <p className="mt-1.5 text-xs leading-relaxed text-faint">
              ...
              Clients send payment straight to this email.
            </p>
          </div>
        )}

        <Button onClick={saveWallet} disabled={busy === 'wallet'} className="w-full">
          {busy === 'wallet' ? 'Saving…' : 'Save payout method'}
        </Button>
        {saved === 'wallet' && <p className="mt-3 text-sm text-verified-text">Saved.</p>}
        <p className="mt-3 text-xs text-faint">While a task is active, your payout method is locked — payment details were snapshotted when you accepted.</p>
      </Card>
