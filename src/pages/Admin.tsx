...
export default function Admin() {
  ...
  // Direct table writes — the schema's protect trigger lets admins through.
  async function review(sub: PendingSub, approve: boolean) {
    setBusyId(sub.id); setError(null)
    const reason = approve ? null : (window.prompt('Rejection reason (internal):') ?? 'Documents unclear')
  ...
  const loadUsers = useCallback(async () => {
    const { data, error: e } = await supabase
      .from('profiles')
      .select('id, display_name, full_name, role, kyc_status, is_banned, is_suspended, open_to_work, created_at')
  ...
  async function setField(u: UserRow, patch: Partial<Pick<UserRow, 'kyc_status' | 'is_banned' | 'is_suspended'>>) {
    setError(null)
    const { error: e } = await supabase.from('profiles').update(patch).eq('id', u.id)
    if (e) { setError(e.message); return }
    await loadUsers()
  }
  ...
  async function toggleBan(u: UserRow) {
    ...
    await setField(u, { is_banned: !u.is_banned })
  }
              ...
              <Card key={sub.id} className="p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    {open === sub.id ? (
                      <>
                        <Button variant="ghost" onClick={() => review(sub, false)} disabled={busyId === sub.id}>Reject</Button>
                        <Button onClick={() => review(sub, true)} disabled={busyId === sub.id}>{busyId === sub.id ? 'Saving…' : 'Approve'}</Button>
                      </>
                    ) : (
              ...
              <div key={u.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-hair py-3 last:border-b-0">
                <div className="min-w-0">
                  <p className="truncate text-sm text-ink">
                    ...
                    {u.is_suspended && !u.is_banned && <span className="ml-2 font-mono text-[11px] uppercase tracking-wider text-pending-text">paused</span>}
                  </p>
                  <p className="mt-0.5 font-mono text-xs text-faint">
                    {u.open_to_work ? 'open to work' : 'not open'} · joined {new Date(u.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {!isSelf && (
                    <>
                      {u.kyc_status === 'verified'
                        ? <Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => setField(u, { kyc_status: 'none' })}>Unverify</Button>
                        : <Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => setField(u, { kyc_status: 'verified' })}>Verify</Button>}
                      <Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => setField(u, { is_suspended: !u.is_suspended })}>
                        {u.is_suspended ? 'Resume' : 'Pause'}
                      </Button>
