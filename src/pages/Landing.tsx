...
function Steps() {
  ...
  return (
    <div className="relative mt-12">
      {/* 桌面横轨 */}
      <div className="absolute left-0 right-0 top-[14px] hidden h-px bg-hair md:block" />
      <div
        className="absolute left-0 top-[14px] hidden h-px bg-petrol md:block"
        style={{ width: `${(act / 3) * 100}%`, transition: 'width .7s var(--ease-ld)' }}
      />
      {/* 移动竖轨 */}
      <div className="absolute bottom-6 left-[14px] top-2 w-px bg-hair md:hidden" />
      <div
        className="absolute left-[14px] top-2 w-px bg-petrol md:hidden"
        style={{ height: `${(act / 3) * 86}%`, transition: 'height .7s var(--ease-ld)' }}
      />
      <div className="grid gap-9 md:grid-cols-4 md:gap-6">
        {STEPS4.map(([n, h, b], i) => (
          ...
          >
            <span className={`absolute left-0 top-0 flex h-7 w-7 items-center justify-center rounded-full border font-mono text-[10px] font-bold transition-colors duration-500 md:relative md:mb-3 md:inline-flex ${node(i)}`}>
              {n}
            </span>
            <p className={`font-display text-[17px] font-medium tracking-tight transition-colors duration-500 ${i === act ? 'text-ink' : 'text-muted'}`}>
              {h}
            </p>
  ...
  return (
    <div className="min-h-screen bg-paper text-ink">
      ...
      <section className="relative overflow-hidden">
        ...
        <Rv as="h2" delay={60} className="t-h2 mt-4 font-display font-medium">
          Paid ≠ done. <span className="text-petrol">You close every task.</span>
        </Rv>
        <Rv delay={140} className="relative mt-10 overflow-hidden rounded-2xl bg-petrol p-8 shadow-[0_20px_56px_rgba(36,75,77,0.32)] sm:p-10">
          ...
          <div className="mt-8 border-t border-paper/20 pt-6">
            <p className="font-display text-xl font-medium tracking-tight text-paper sm:text-2xl">
              We never ask you for money. <span className="text-paper/70">No fees, no deposits, no starter kits — anyone who asks isn't us.</span>
            </p>
          </div>
        </Rv>
        </div>
        ...
        <div className="mx-auto max-w-5xl px-5 py-20">
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-3 sm:gap-8">
            {[
              ...
              ['SSN check', 'Identity only — admin-access only.'],
