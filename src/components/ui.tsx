...
const TASK_BADGE_ZH: Record<TaskStatus, string> = {
  unassigned: '开放中',
  offered: '已发邀约',
  in_progress: '进行中',
  under_review: '审核中',
...
export function TaskBadge({ status, lang = 'en' }: { status: TaskStatus; lang?: 'en' | 'zh' }) {
  const b = TASK_BADGE[status] ?? { s: 'unverified' as const, label: status }
  const label = lang === 'zh' ? (TASK_BADGE_ZH[status] ?? b.label) : b.label
  return <StatusBadge status={b.s} label={label} />
}
...
export function Linkified({ text, invert = false }: { text: string; invert?: boolean }) {
  const parts = text.split(URL_RE)
  return (
    <span className="whitespace-pre-wrap break-words">
      {parts.map((p, i) =>
        i % 2 === 1 ? (
          <a key={i} href={p} target="_blank" rel="noreferrer" className={`${invert ? 'text-paper' : 'text-petrol'} underline underline-offset-2 break-all`}>{p}</a>
        ) : (
          <span key={i}>{p}</span>
        ),
      )}
...
/** FilePick(v85.5):隐藏原生 file input + 自绘英文按钮 —— 系统语言免疫。
 *  已收敛 2 处(OnboardingKYC/TaskDetail);其余上传点下批迁移。 */
export function FilePick({ id, accept, multiple, onFiles, label = 'Choose file', empty = 'No file selected', selectedText }: {
  id: string
  accept?: string
  multiple?: boolean
  ...
  return (
    <span className="inline-flex items-center">
      ...
      <label htmlFor={id}
        className="inline-flex cursor-pointer items-center rounded-lg border border-hair bg-white px-3 py-2 font-display text-sm text-ink transition hover:bg-paper">
        {label}
      </label>
      {!selectedText && <span className="ml-3 font-mono text-xs text-muted">{empty}</span>}
    </span>
  )
}
