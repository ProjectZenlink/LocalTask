...
/** Hero 主秀 v3(v85.9):双任务交替,全区域定高零回流。
 *  脚本:落桌 → 字段显影 → OFFER → 进行中 → 已提交 → 已付款(直达钱包)→ CLOSED 钢印 → 淡出,换下一单。 */

const PHASE_MS = [950, 1700, 2100, 1900, 2400, 2900, 650]
const STEPS = ['OFFER', 'ACTIVE', 'SUBMITTED', 'PAID', 'CLOSED'] as const

const TASKS = [
  { id: 'TASK A-1042', amt: '$120.00', time: '~45 min', payout: 'Stablecoin' },
  { id: 'TASK B-2087', amt: '$85.00', time: '~30 min', payout: 'e-Wallet' },
] as const
...
export default function TaskCardDemo() {
  ...
  return (
    <div className="relative mx-auto w-full max-w-md">
      ...
      <div
        key={loop}
        className={`relative overflow-hidden rounded-2xl border border-hair bg-surface p-6 shadow-[0_24px_64px_rgba(30,29,25,0.10)] ${phase === 0 ? 'ld-rise' : ''} ${phase === 6 ? 'ld-out' : ''}`}
      >
        {/* 头行(定高) */}
        ...
        <div className="mt-4 grid h-[92px] grid-cols-2 content-start gap-x-6 gap-y-3">
          {[
            ...
            <div key={k} className="ld-fade" style={{ animationDelay: `${240 + i * 90}ms` }}>
              <p className="flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.18em] text-faint">
                {k}
                {k === 'PAYOUT' && phase >= 2 && phase < 6 && (
                  <span className="ld-pop inline-flex items-center gap-0.5 rounded border border-petrol/30 bg-petrol/10 px-1 py-px font-mono text-[8.5px] normal-case tracking-wider text-petrol" title="Snapshotted when you accept — later profile edits never touch this task.">
        ...
        <div className="ld-fade mt-5 flex h-7 items-center gap-1.5 overflow-hidden" style={{ animationDelay: '640ms' }}>
          {STEPS.map((s, i) => (
            <span
              key={s}
              className={`whitespace-nowrap rounded-full border px-1.5 py-1 font-mono text-[10px] uppercase tracking-wider transition-all duration-500 sm:px-2 ${
                i < active
                  ? 'border-petrol/25 bg-petrol/10 text-petrol/70'
                  : i === active
                    ? 'border-petrol bg-petrol text-paper'
                    : 'border-hair text-faint/50'
              }`}
        ...
        <div className="mt-5 flex h-14 items-center border-t border-hair">
          {phase < 4 && (
            <p className="font-mono text-xs text-faint">
              {phase <= 1 ? 'Offer expires in 23h.' : phase === 2 ? 'Steps in progress…' : 'Submitted — in review.'}
            </p>
          )}
          {phase === 4 && (
            <p className="ld-fade font-mono text-xs text-muted">Paid · straight to your wallet.</p>
          )}
          {phase >= 5 && (
            <p className="ld-fade font-mono text-xs text-verified-text">✓ Settled. Task closed.</p>
          )}
        </div>
      ...
      <div className="absolute -bottom-5 -right-4 hidden rounded-xl border border-hair bg-surface/75 px-3.5 py-2.5 shadow-[0_10px_28px_rgba(30,29,25,0.10)] backdrop-blur md:block">
        <p className="max-w-[180px] font-mono text-[10.5px] leading-relaxed text-muted">
          Payment goes <span className="text-petrol">straight to your wallet</span> — we never hold your money.
        </p>
      </div>
