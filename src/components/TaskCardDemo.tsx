        ...
        <div className="mt-5 flex h-14 items-center border-t border-hair">
          ...
          {phase >= 4 && phase < 6 && (
            <div className="ld-fade flex w-full items-center justify-between gap-3">
              <p className="min-w-0 truncate font-mono text-xs text-muted">Paid · {task.ref}</p>
              <button
                type="button"
                tabIndex={-1}
                className={`shrink-0 rounded-lg bg-petrol px-3.5 py-2 text-[12px] font-medium text-paper ${phase === 5 ? 'ld-breathe' : ''}`}
              >
                Confirm received
              </button>
            </div>
          ...
          {phase >= 6 && (
            <p className="ld-fade font-mono text-xs text-verified-text">✓ Confirmed by you. Task closed.</p>
          )}
        </div>
