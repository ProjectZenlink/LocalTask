...
import { Inbox, ListChecks, Coins, UserRound } from 'lucide-react'
...
const TABS = [
  ...
  { to: '/earnings', label: 'Coins', icon: Coins },
        ...
        <div className="mx-auto flex max-w-4xl items-center justify-between px-5 py-4">
          ...
          <nav className="flex items-center gap-5 text-sm">
            {session ? (
              isAdmin || isAm ? (
                <>
                  <Link to={isAdmin ? '/admin' : '/am'} className="font-mono text-xs uppercase tracking-wider text-petrol transition hover:text-petrol-hover">{isAdmin ? 'Console' : 'Workspace'}</Link>
                  <button onClick={() => setAskOut(true)} className="text-muted transition hover:text-ink">Sign out</button>
                </>
              ) : (
                <>
                  ...
                  <button onClick={() => setAskOut(true)} className="hidden text-muted transition hover:text-ink sm:inline">Sign out</button>
                </>
              )
            ) : (
              <>
