              ...
              <div className={`mt-2 flex ${mine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[78%] rounded-2xl px-3.5 py-2 text-sm leading-relaxed ${
                  ...
                  {!m.recalled_at && m.body && (!mine && holdingTr(m) ? (
                    ...
                    <p className="whitespace-pre-wrap break-words"><Linkified text={tr && !showO ? tr : m.body} invert={mine} /></p>
                  ))}
                  {m.kind === 'auto' && (
                    <p className={`mt-1 font-mono text-[10px] uppercase tracking-[0.14em] ${mine ? 'text-paper/70' : 'text-faint'}`}>· {t.autoTag}</p>
                  )}
                  {tr && (
                    <button
                      onClick={() => setShowOrig(p => ({ ...p, [m.id]: !p[m.id] }))}
                      className="mt-1 block font-mono text-[10px] uppercase tracking-[0.14em] text-faint transition hover:text-petrol"
                    >
