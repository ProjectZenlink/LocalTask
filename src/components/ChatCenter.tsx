...
const COPY = {
  zh: {
    ...
    orig: '看原文', trBack: '看译文', read: '已读', profileBtn: '客户资料', attach: '发送图片/文件', attErrType: '仅支持 图片(jpg/png/webp/gif) 与 PDF。', attErrSize: '文件不能超过 10MB。', attErrUp: '上传失败,请重试。', autoTag: '自动回复',
  },
  en: {
    ...
    orig: 'Original', trBack: 'Translation', read: 'Read', profileBtn: 'Customer info', attach: 'Send image / file', attErrType: 'Images (jpg/png/webp/gif) and PDF only.', attErrSize: 'Max file size is 10MB.', attErrUp: 'Upload failed — please retry.', autoTag: 'Auto-reply',
  },
}
...
export default function ChatCenter({ lang, myRole, variant = 'page' }: {
  ...
  const reqAtRef = useRef<Map<string, number>>(new Map())
  ...
  useEffect(() => { requestedRef.current = new Set(); reqAtRef.current = new Map(); setTrs({}); setShowOrig({}) }, [lang])
  ...
  useEffect(() => {
    ...
    const now = Date.now()
    for (const m of pending) { requestedRef.current.add(m.id); reqAtRef.current.set(m.id, now) }
  ...
  /** v65 ③:收信方等译文 —— 已请求且未超 2.5s 且译文未到 → 先亮加载点,不闪原文 */
  const holdingTr = (m: Msg) => {
    const at = reqAtRef.current.get(m.id)
    return at !== undefined && !trs[m.id] && Date.now() - at < 2500
  }
