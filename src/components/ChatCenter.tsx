        ...
        {convs.map(c => (
          <button key={c.id} onClick={() => setSel(c.id)}
            ...
            <span className="flex items-center gap-3">
              <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full font-display text-[11px] font-medium ${AVATAR[c.other_role]}`}>
  ...
  const thread = (
    <div className="flex h-full min-w-0 flex-col">
      <div className="flex h-12 shrink-0 items-center gap-3 border-b border-hair px-4">
        ...
        {cur && (
          <>
            <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full font-display text-[10px] font-medium ${AVATAR[cur.other_role]}`}>
              {initials(cur.other_name)}
            </span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        ...
        {items.map(it =>
          it.kind === 'day' ? (
            ...
            <div key={it.key} className={`ld-fade flex ${it.mine ? 'justify-end' : 'justify-start'} ${it.groupEnd ? 'mb-3' : 'mb-1'}`}>
              <div className="max-w-[74%]">
                ...
                {it.groupEnd && (
                  <p className={`mt-1 font-mono text-[9.5px] text-faint ${it.mine ? 'text-right' : 'text-left'}`}>
                    {bubbleTime(it.m.created_at)}
                  </p>
                )}
              </div>
            </div>
                ...
                <div className="max-h-72 overflow-y-auto">
                  {filteredTargets.map(x => (
                    <button key={x.id} onClick={() => void startWith(x.id)}
                      ...
                      <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full font-display text-[10px] font-medium ${AVATAR[x.role === 'freelancer' ? 'user' : x.role] ?? AVATAR.user}`}>
                        {initials(x.name)}
                      </span>
