            ...
            <div key={m.id}>
              ...
              <div className={`flex items-center ${mine ? 'justify-end' : 'justify-start'} ${gapPrev && !newDay ? 'mt-3' : 'mt-[3px]'}`}
                {...(mine && myRole !== 'user' && !m.recalled_at ? {
                  onContextMenu: (e: React.MouseEvent) => { e.preventDefault(); void recallMsg(m) },
                  onTouchStart: (e: React.TouchEvent) => {
                    const tgt = e.target as HTMLElement
                    const timer = window.setTimeout(() => void recallMsg(m), 500)
                    const clear = () => window.clearTimeout(timer)
                    tgt.addEventListener('touchend', clear, { once: true })
                    tgt.addEventListener('touchmove', clear, { once: true })
                  },
                ...
                {mine && myRole !== 'user' && !m.recalled_at && (
                  <button onClick={() => void recallMsg(m)} title={t.recall}
                    className="mr-1.5 hidden h-6 w-6 shrink-0 items-center justify-center rounded-full border border-hair bg-white text-faint transition hover:text-danger-text group-hover:flex">
                    <Undo2 size={11} />
                  </button>
                )}
