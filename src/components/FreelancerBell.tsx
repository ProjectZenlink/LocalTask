...
const COPY = {
  en: {
    offers: (n: number, h: number) => `${n} pending offer${n > 1 ? 's' : ''} — soonest expires in ~${h}h`,
  ...
  zh: {
    offers: (n: number, h: number) => `${n} 个待处理邀约，最早约 ${h} 小时后过期`,
...
export default function FreelancerBell() {
  ...
  const load = useCallback(async () => {
    ...
    if (offers.length > 0) {
      ...
      out.push({ key: 'offers', text: t.offers(offers.length, hrs), to: '/offers' })
    }
    ...
    if (recs > 0) out.push({ key: 'review', text: t.review(recs), to: '/accounts' })
