...
import { getDockOpen, setDockOpen as storeSetDockOpen, subscribeDock } from '../lib/dockState'
...
export default function MessagesDock({ myRole }: { myRole: 'am' | 'admin' }) {
  ...
  const [open, setOpen] = useState(getDockOpen)
  ...
  useEffect(() => {
    const onOpen = (e: Event) => {
      ...
      setWant(w); storeSetDockOpen(true)
    }
  ...
  if (!open) {
    return (
      <button onClick={() => storeSetDockOpen(true)} title={t.open}
  ...
  return (
    <div className="fixed bottom-24 right-0 top-20 z-30 flex w-[440px] max-w-[94vw] flex-col overflow-hidden rounded-l-2xl border border-r-0 border-hair bg-surface shadow-[0_16px_48px_rgba(26,32,30,0.22)]">
      <div className="min-h-0 flex-1 bg-white/40">
        <ChatCenter lang={lang} myRole={myRole} variant="dock"
          initialWith={want} onClose={() => { storeSetDockOpen(false); setWant(null) }} />
      </div>
    </div>
  )
}
